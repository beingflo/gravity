...
impl Particle {
    ...
    pub fn draw(&self, draw: &Draw, camera: &Camera, vel_indicator: bool, accel_indicator: bool, viewport_width: f32, viewport_height: f32) {
        ...
        if self.pos.x >= ul.x && self.pos.x <= lr.x && self.pos.y >= lr.y && self.pos.y <= ul.y {
            ...
            if vel_indicator {
                draw.line().start(pos).end(pos + (self.vel.normalize() * self.vel.magnitude().log(2.0) * camera.zoom)).thickness(1.0 * camera.zoom).caps_round().color(BLUE);
            }

            if accel_indicator {
                draw.line().start(pos).end(pos + (self.accel.normalize() * self.accel.magnitude().log(2.0) * camera.zoom)).thickness(1.0 * camera.zoom).caps_round().color(RED);
            }
        }
    }
